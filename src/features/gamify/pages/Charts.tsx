...
const songYearOf = (s: any): number | undefined => {
  if (!s || !s.created_at) return undefined;
...
export default function Charts() {
  ...
  const years = [...new Set(allSongs.map((s) => songYearOf(s)).filter((y): y is number => y !== undefined))].sort((a, b) => b - a)
  ...
  const yearSongs = year === new Date().getFullYear() ? weekly : allSongs.filter((s) => songYearOf(s) === year)
