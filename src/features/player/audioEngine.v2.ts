...
import { vocalGains, type VocalMode } from './lib/vocalGains';
...
// NOTE: bassFilter never connects directly to master gain — the vocal
// (mid/side) stage owns that path. See createEffects/createVocalStage above.
export function setVocalMode(nodes: { midGain: GainNode; sideGain: GainNode }, mode: VocalMode) {
  const [mid, side] = vocalGains(mode);
  nodes.midGain.gain.value = mid;
  nodes.sideGain.gain.value = side;
}
